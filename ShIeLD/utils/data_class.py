...
def pad_or_repeat_rows(x: torch.Tensor, fix_nodeSize: int) -> torch.Tensor:
    ...
    if n >= fix_nodeSize:
        return x[:fix_nodeSize]  # truncate if larger

    # Compute how many times we need to repeat
    repeat_times = fix_nodeSize // n
    remainder = fix_nodeSize % n

    # Repeat whole tensor
    repeated = x.repeat((repeat_times, 1))

    # Add the remaining rows
    if remainder > 0:
        repeated = torch.cat([repeated, x[:remainder]], dim=0)

    return repeated


class fixed_dataset(Dataset):
    """
    Custom PyTorch Geometric dataset for loading diabetes-related graph data.

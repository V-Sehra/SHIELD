...
def main():
    ...
    requirements["path_training_results"] = Path(
        str(requirements["path_training_results"]) + f"_{args.model_type}"
    )
    requirements["path_to_model"] = Path(
        str(requirements["path_to_model"]) + f"_{args.model_type}"
    )
    ...
    data_loader_train, data_loader_validation = optuna_utils.load_data_loader(
    ...
    db_path = Path(
        f"{requirements['path_training_results']}/tuning_v{args.validationSplit}.db"
    ).resolve()
    ...
    # hypersearch:
    study.optimize(
        lambda trial: optuna_utils.objective(
            trial,
